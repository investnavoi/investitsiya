...
function renderInlineProductSection(section){
  ...
  var rowsHtml = filteredProds.length ? filteredProds.map(function(p,i){
    ...
    var compToggle = '<span class="prod-comp-toggle" onclick="event.stopPropagation();toggleProductComposition(\''+compRowId+'\', this)" data-row-id="'+compRowId+'" style="display:inline-flex;align-items:center;gap:6px;margin-top:5px;font-size:.75rem;font-weight:700;color:#465fff;cursor:pointer;user-select:none;padding:4px 8px;border-radius:6px;transition:all .2s" onmouseover="this.style.background=\'rgba(70,95,255,.08)\'" onmouseout="this.style.background=\'\'">' +
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>' +
        'Tarkibi <span class="prod-comp-caret" style="font-size:.8rem">▾</span></span>';
...
function populateProductSelects(){
  ...
  try {
    if(typeof getProductPriorityMap === 'function'){
      ...
      console.log('[priority dropdown v25] map byId='+_byIdN+' byHs='+_byHsN, _pm);
      ...
      if(_sample) console.log('[priority dropdown v25] namuna 251611 → letter:', getProductPriorityLetter(_sample), 'id:', _sample.id, 'hs:', _sample.hs_code);
    } else {
      console.warn('[priority dropdown v25] getProductPriorityMap funksiyasi YO\'Q — eski import-analysis.js yuklangan?');
    }
  } catch(_pe){ console.warn('[priority dropdown v25] diagnostika xato:', _pe); }
  ...
  if(dd){
    ...
    ['build','azot','other'].forEach(function(section){
      ...
      sectionRaws.forEach(function(raw){
        ...
        var rawNameFull = raw.name_uz || raw.name_en || 'Nomsiz';
        var rawName = (typeof cleanRawMaterialLabel === 'function') ? cleanRawMaterialLabel(rawNameFull) : rawNameFull;
        html += '<div class="csd-group csd-group-raw" data-rawid="'+raw.id+'">';
        html += '<div class="csd-group-head csd-raw-head" onclick="toggleCsdGroup(this)" title="'+escapeHtmlText(rawNameFull)+'"><span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#465fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>'+escapeHtmlText(rawName)+'</span><span class="csd-count">'+rawProds.length+'</span><span class="csd-caret">▾</span></div>';
